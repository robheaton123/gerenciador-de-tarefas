...
function gerarUltimoItem(numPaginas) {
    return (
        <Pagination.Last
        key="pagLast"
        onClick={() => props.mudarPagina(numPaginas)} //numPaginas é a ultima pagina
        disabled={props.paginaAtual === numPaginas} /> //se a pagina atual for igual a ultima
    );
}

function obterPaginacao() {
    //total de itens dividido por itens que devem ficar na pagina //aredondado para integer
